"""Train on a Modal GPU and bring the results back.
    ...
    uv run --with modal modal run modal_run.py --model ntm-ff --sequences 200000
...
app = modal.App("ntm-copy-task", image=image)


@app.function(gpu="A10G", timeout=8 * 3600, volumes={"/root/results": volume})
def train_on_gpu(model: str, sequences: int, seed: int):
    import sys
...
def main(model: str = "lstm", sequences: int = 1_000_000, seed: int = 0):
    import pathlib

    train_on_gpu.remote(model, sequences, seed)

    local = pathlib.Path("results/copy") / model
    local.mkdir(parents=True, exist_ok=True)
    for name in ("log.csv", "model.pt", "run.json"):
        with open(local / name, "wb") as f:
            for chunk in volume.read_file(f"copy/{model}/{name}"):
                f.write(chunk)
    print(f"downloaded to {local}/  — now run: uv run main.py plot --model {model}")
