...
The checks read the episode back out of x: the item delimiters say where the items are, the
query delimiter says where the query is, and the target has to be whichever item followed the
one repeated as the query.
"""
...
def test_shapes_match_the_declared_channel_counts():
    ...
    timesteps = data.timesteps(items=3)  # 3 delimited items + a closing one, 5 query steps, 3 answer
...
def test_an_episode_of_N_items_carries_N_plus_2_item_delimiters():
    for items in range(data.MIN_ITEMS, data.MAX_ITEMS + 1):
        x, _, _ = data.batch(2, items, items)

        assert int(x[0, :, ITEM_DELIMITER].sum()) == items + 2  # N + 1 for the list, 1 for the query
        assert int(x[0, :, QUERY_DELIMITER].sum()) == 1
        assert len(items_of(x[0])) == items
...
def test_the_target_holds_across_a_whole_batch_of_independent_queries():
    x, target, _ = data.batch(64, 5, 5)

    for episode, answer in zip(x, target):
        items = items_of(episode)
        assert any(torch.equal(answer[-STEPS:], items[index + 1]) for index in queried_indices(episode))
...
def test_probe_cases_run_at_the_trained_maximum_and_twice_it():
    cases = dict(data.PROBE_CASES)
    assert len(items_of(cases["trained"]()[0][0])) == data.MAX_ITEMS == 6
    assert len(items_of(cases["long"]()[0][0])) == 12  # paper Figure 11: twice the training length
